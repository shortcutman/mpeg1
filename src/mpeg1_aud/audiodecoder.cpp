...
namespace {
    ...
    const std::array<std::vector<int32_t>, 32> Level_For_Index_Subband = {{
    ...
    const std::map<int32_t, QuantClass> QuantClasses = {
        {3, {true, 5}},
        {5, {true, 7}},
        {7, {false, 3}},
        {9, {true, 10}},
    ...
    const std::array<std::array<int, 32>, 64> N = gen_nik_table();
    ...
    for (size_t sb = 0; sb < sblimit; sb++) {
        for (size_t ch = 0; ch < channels; ch++) {
            if (allocations[ch][sb]) {
                ...
                if (channels == 1) {
                    scalefactors[1][sb][0] = scalefactors[0][sb][0];
                    scalefactors[1][sb][1] = scalefactors[0][sb][1];
                    scalefactors[1][sb][2] = scalefactors[0][sb][2];
                }
            }
        }
    }
            ...
            for (size_t sIdx = 0; sIdx < 3; sIdx++) { // sample index for each in the granule
                ...
                for (size_t ch = 0; ch < 2; ch++) {
                    ...
                    for (size_t i = 0; i < 8; i++) {
                        for (size_t j = 0; j < 32; j++) {
                            U[i * 64 + j] = _V[ch][(table_idx + i * 128 + j) & 1023];
                            U[i * 64 + 32 + j] = _V[ch][(table_idx + i * 128 + 96 + j) & 1023];
                        }
                    }
